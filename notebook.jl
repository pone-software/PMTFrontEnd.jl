...
5) Use NNLS to unfold digitizes waveform into reconstructed pulses
"""
...
# ╔═╡ cd937210-5d81-48ff-a885-19d7618e807f


# ╔═╡ 1adb8eb9-dd29-4eb5-af04-9d62b044b9ba
md"""
# Parameters for the pulse template
Gumbel width: $(@bind gumbel_width Slider(1:0.1:5, default=3, show_value=true))
...
plot(x -> pdf(pulse(0), x) * pulse_model.amplitude, -50, 50, ylabel="Amplitude (a.u.)", xlabel="Time (ns)")
...
Noise amplitude: $(@bind noise_amp Slider(1:0.1:5, default=3, show_value=true))
"""
...
begin
	Random.seed!(1337)
	arrival_time_pdf = Gamma(2., 2.)
	hit_times =rand(arrival_time_pdf, 20)
	
	charges = rand(spe_d, size(hit_times))
	
	#wf_model = make_gumbel_waveform_model(hit_times)
	wf = PMTWaveform(hit_times, charges, pulse_model)
end
...
begin
	...
	waveform_values = evaluate_waveform(timestamps_hires, wf)
	waveform_values_noise = add_gaussian_white_noise(waveform_values, noise_amp)
...
begin
	...
	plot!(timestamps, digi_wf, label="Digitized Waveform")
	sticks!(twinx(), hit_times, charges, legend=false, left_margin = 30Plots.px, ylabel="Charge (PE)", ylim=(1, 10), color=:green)
end
...
Initial pulse resolution (ns): $(@bind unf_pulse_res Slider(0.1:0.1:3, default=1., show_value=true))
...
begin
	...
	pulse_charges = apply_nnls(pulse_times, pulse_model, timestamps, digi_wf)
	
	nonzero = pulse_charges .> 0.1
	
	reco_wf = PMTWaveform(pulse_times[nonzero], pulse_charges[nonzero], pulse_model)
	
	plot(timestamps_hires, waveform_values_noise, label="Waveform + Noise", xlabel="Time (ns)", ylabel="Amplitude (a.u.)", right_margin = 45Plots.px,
	xlim=(-50, 50))
	plot!(timestamps, digi_wf, label="Digitized Waveform")
	plot!(timestamps, evaluate_waveform(timestamps, reco_wf), label="Reconstructed Waveform")
	
	sticks!(twinx(), pulse_times, pulse_charges, legend=false, left_margin = 30Plots.px, ylabel="Charge (PE)", ylim=(1, 10), color=:green)
end
